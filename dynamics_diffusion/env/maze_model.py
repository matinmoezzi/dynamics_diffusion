...
def point_maze(maze_str):
    ...
    # Sampled particle to demonstrate the diffrence between sampled next_state and true next_state
    particle = worldbody.body(name="sampled_particle", pos=[1.2, 1.2, 0])
    particle.geom(
        name="sampled_particle_geom",
        type="sphere",
        size=0.1,
    ...
    particle.site(
        name="sampled_particle_site", pos=[0.0, 0.0, 0], size=0.2, rgba="0 0 1 1"
    )
    particle.joint(name="sampled_ball_x", type="slide", pos=[0, 0, 0], axis=[1, 0, 0])
    particle.joint(name="sampled_ball_y", type="slide", pos=[0, 0, 0], axis=[0, 1, 0])
...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    ...
    def reset_model(self):
        ...
        reset_location = np.tile(reset_location, 2)
