...
def get_model_fn(model, train=False):
    ...
    def model_fn(x, labels, model_kwargs):
        ...
        if not train:
            model.eval()
            return model(x, labels, **model_kwargs)
        else:
            model.train()
            return model(x, labels, **model_kwargs)
...
def get_score_fn(sde, model, model_kwargs, train=False, continuous=False):
...
def shared_predictor_update_fn(
    x, t, sde, model, model_kwargs, predictor, probability_flow, continuous
):
    """A wrapper that configures and returns the update function of predictors."""
    score_fn = get_score_fn(
        sde, model, model_kwargs, train=False, continuous=continuous
    )
...
def shared_corrector_update_fn(
    x, t, sde, model, model_kwargs, corrector, continuous, snr, n_steps
):
...
def get_ode_sampler(
    ...
    def denoise_update_fn(model, x):
        score_fn = get_score_fn(sde, model, model_kwargs, train=False, continuous=True)
