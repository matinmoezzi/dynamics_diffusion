...
class Diffusion(nn.Module):
    def __init__(
        ...
        alphas_cumprod_prev = alphas_cumprod_prev.to(self.device)
    ...
    def p_mean_variance(self, x, t, cond):
        ...
        if self.clip_denoised:
            x_recon.clamp_(-self.max_x, self.max_x)
        else:
            assert RuntimeError()
    ...
    # @torch.no_grad()
    def sample(self, cond, *args, **kwargs):
        batch_size = cond.shape[0]
        shape = (batch_size, self.x_dim)
        x = self.p_sample_loop(cond, shape, *args, **kwargs)
        if self.clip_denoised:
            return x.clamp_(-self.max_x, self.max_x)
        return x
